...
for (i in 1:length(cols)) {
  ...
  bh_opt <-
    optim(
      ...
      control = c(factr = 1e-15, factr = 1e-15 * max(Y) / mean(Y / X))
    )
  hs_opt <-
    optim(
      hs_p_guess,
      ...
      method = "L-BFGS-B",
      lower = c(1.00001, mean(Y)),
      upper = c(min(r_max, max(Y) / min(X)), max(Y)),
      control = c(factr = 1e-15, factr = 1e-15 * max(Y) / mean(Y / X))
    )
  rck_opt <-
    optim(
  ...
  r2_rck <-
    append(r2_rck, r_squared(RI(rck_opt_r, rck_opt_k, X), Y))
  ...
  opt_esc_pt_scaled = append(opt_esc_pt_scaled, k_pt[i] * (1 / (1 + phi)) ^
                               (1 / phi))
  ...
  if (r2_rck[i] == max(c(r2_bh[i], r2_rck[i], r2_hs[i], r2_pt[i]))) {
    ...
    opt_esc_rck_scaled_bf <-
      append(opt_esc_rck_scaled_bf, opt_esc_rck_scaled[i])
  }
...
#### Figures ####
if (which_data_set == 2) {
  #only prints figures for non-deterministic data
  ...
  pdf(file = './plots/scatter_plot_bh_hs_r.pdf',
      width = 3.5,
      height = 3.5)
  par(
    mfrow = c(1, 1),
  ...
  pdf(file = './plots/scatter_plot_bh_hs_k.pdf',
      width = 3.5,
      height = 3.5)
  par(
    mfrow = c(1, 1),
  ...
  pdf(file = './plots/opt_esc_hist_prop_of_fitted_k.pdf',
      width = 3.5,
      height = 3.5)
  ...
  pdf(file = './plots/opt_esc_hist_prop_of_max_pop.pdf',
      width = 3.5,
      height = 3.5)
  par(
    mfrow = c(4, 1),
  ...
  for (j in tol) {
    ...
    for (i in 1:num_cols) {
      ...
      else if (r2_rck[i] == max(c(r2_bh[i], r2_rck[i], r2_hs[i], r2_pt[i])) &
               r_rck[i] < j) {
        num_rck = num_rck + 1
      }
      else if (r2_hs[i] == max(c(r2_bh[i], r2_rck[i], r2_hs[i], r2_pt[i])) &
               r_hs[i] < j) {
        num_h_s = num_h_s + 1
      }
      else if (r2_pt[i] == max(c(r2_bh[i], r2_rck[i], r2_hs[i], r2_pt[i])) &
               r_pt[i] < j) {
        num_p_t = num_p_t + 1
      }
    }
  ...
  pdf(file = './plots/cumulative_frequency_plot.pdf',
      width = 3.5,
      height = 3.5)
  par(
    mfrow = c(1, 1),
